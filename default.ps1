properties {
	...
	$sln_file = "$src_directory\Thinktecture.IdentityServer.v3.AspNetIdentity.sln"
...
task CreateNuGetPackage -depends Compile {
	...
	md $dist_directory
	exec { . $nuget_path pack $src_directory\AspNetIdentityUserService\AspNetIdentityUserService.csproj -o $dist_directory -version $packageVersion }
}
